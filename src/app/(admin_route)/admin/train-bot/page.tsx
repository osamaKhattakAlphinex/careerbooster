...
export interface SlidingPanelRef {
  openModal: (arg1: boolean, arg2) => void;
}
const TrainRegistrationBotAdminPage = () => {
  ...
  const slidingPanelRef: React.MutableRefObject<SlidingPanelRef | null> =
    useRef(null);
  ...
  const actions: TableAction[] = [
    {
      name: "review",
      type: "handler",
      element: (rec) => handleViewClick(rec),
      styles:
  ...
  const handleTuneModel = async (ids: string[] | []) => {
    ...
    try {
      ...
      if (success) {
        const formattedData = reviewedData.map((rec) => ({
          messages: [
        ...
        const jsonl = formattedData
          .map((obj) => JSON.stringify(obj, null, 0))
          .join("\n");
  ...
  const handleDownload = async () => {
    ...
    if (records) {
      const data = records.map((rec: TrainBotEntryType) => {
        return {
          messages: [
            { role: "user", content: rec.input },
            { role: "assistant", content: rec.idealOutput },
