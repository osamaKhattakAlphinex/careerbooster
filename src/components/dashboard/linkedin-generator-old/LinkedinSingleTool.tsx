...
const LinkedinSingleTool: React.FC<LinkedinSingleToolProps> = ({
  ...
  return (
    <>
      <div className=" bg-[#222027] py-8 px-6 flex align-center gap-5 justify-center items-center rounded-[10px] mb-[20px]">
        <div
        ...
        <div className=" flex flex-col w-[24.0625rem] gap-2 ml-2">
        ...
        >
          ...
          <span
            ...
            className={`${buttonTextColor} text-[15px] font-semibold`}
          >
            {buttonText}
          </span>
        </button>
      </div>
      {/* Display the box if it's visible */}
      {isBoxVisible && (
        <div className="w-20 h-20 border-2">{textOfPackageBadge}</div>
      )}
    </>
  );
};
