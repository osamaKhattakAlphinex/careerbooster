...
const DashboardBot = () => {
  ...
  const focusTool = (audio: any, focusedElement: string):string => {
  ...
  const handleClick = async () => {
    try {
      if (isGif) {
        setIsGif(false);
        if (isAudioPlaying && componentRef.current) {
  ...
  useEffect(() => {
    ...
    if (audioBuffers.length > 0 && audioCounter < audioBuffers.length) {
      const audioUrl: string = focusTool(
    ...
    if (audioCounter !== 0 && audioCounter === audioBuffers.length && componentRef.current) {
      setShowBot(false);
      localStorage.setItem("botHidden", "true");
      componentRef.current.pause();
      removeStyles();
  ...
  return (
    <div
      ref={(ref) => (tourBotRef.current = ref)}
