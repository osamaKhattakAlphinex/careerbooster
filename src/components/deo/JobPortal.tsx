...
import { SingleJob, UserDataType } from "../../../types/ObjTypes/types";
import { ApplicantProfile } from "../public-pages/find-jobs/SingleJobCard";
...
const JobPortal = () => {
  ...
  const [records, setRecords] = useState<SingleJob[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const [open, setOpen] = useState<boolean>(false);
  const [currentRecord, setCurrentRecord] = useState<SingleJob |null>(null);
  ...
  const [deo, setDeo] = useState<UserDataType>();
  ...
  const editJobHandler = (rec: SingleJob) => {
    setCurrentRecord(rec);
    setOpen(true);
  };
  const statusHandler = (rec: Job, newStatus: string) => {
  ...
  const featuredHandler = (featured: boolean, rec: Job) => {
    const c = window.confirm(
  ...
  const deleteJobHandler = (rec: SingleJob) => {
  ...
  const columns = [
    ...
    columnHelper.accessor("link", {
      ...
      cell: (info) => {
        if (info.renderValue()) {
          const url = info.renderValue() as Url
          return (
            <Link
              href={url}
  ...
  const actions: TableAction[] = [
    {
      name: "Edit",
      type: "handler",
      element: (rec: SingleJob) => editJobHandler(rec),
      styles:
    ...
    },
    {
      name: "Delete",
      type: "handler",
      element: (rec:SingleJob) => deleteJobHandler(rec),
      styles:
    ...
    },
    {
      ...
      type: "handler",
      element: (rec:SingleJob) => showApplicantProfiles(rec.applicationProfiles),
  ...
  const selectUsersLimit = (e:React.ChangeEvent<HTMLSelectElement>) => {
    setCurrentPage(1);
    setLimitOfRecords(parseInt(e.target.value,10));
  };

  const showApplicantProfiles = (records: ApplicantProfile[]) => {
