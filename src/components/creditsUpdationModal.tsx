...
import { CreditsUpdationRef } from "@/app/(admin_route)/admin/users/page";
...
const CreditsUpdationModal = forwardRef<CreditsUpdationRef,Props>((props, ref) => {
  const [openCreditsUpdationModal, setOpenCreditsUpdationModal] =
    useState<boolean>(false);
  const [user, setUser] = useState<UserCredits>();
    ...
    >
      <div className="relative w-full max-w-xl max-h-full p-4">
        <div className="relative p-4 text-center rounded-lg shadow dark:bg-gray-800 sm:p-5">
          <h1 className="text-2xl font-bold text-white">Enter Credits</h1>
